function [MDG_field, MDG_field_dir, sources_ind, I_v_bar, signed_MDG_field] = compute_MDG_vector_field(img)
...
dist_direction =  sources_ind - dist_matrix ;
direction_mag = sum(dist_direction.^2, 2);
dist_denom = reshape(direction_mag, [X Y Z]); 
MDG_field_dir = dist_direction ./ repmat(direction_mag,1, 3);
MDG_field = (I_v_bar - img)./ dist_denom;

signed_MDG_field =  MDG_field_dir .* repmat(MDG_field(:), 1, 3);
